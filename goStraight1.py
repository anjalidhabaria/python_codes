...
class GoStraight1():
	...
	gain = 2
        # initiliaze
        rospy.init_node('GoStraight1', anonymous=False)
	...
	# as long as you haven't ctrl + c keeping doing...
		...
		z_err = self.z_dez/self.z_curr

		x=+delta_x
		y+=delta_y
		th=cmath.phase(z_err)
		self.omega = gain*th
		self.saturate(self.omega)
		odom_quat = tft.quaternion_from_euler(0, 0, 0)  # or try putting th instaed of 0 here
		...
		odom.twist.twist = Twist(Vector3(vx, vy, 0), Vector3(0, 0, self.omega))
    ...
    def callback(odom_data):
        ...
        yaw = tft.quaternion_to_yaw(q)
        self.z_curr = cmath.rect(1,yaw)
